...
from game import llm_prompts, strategies   # llm_prompts is pure text — no network
...
def main(argv=None):
    ...
    p.add_argument("--llm-prompt", default=llm_prompts.DEFAULT_VERSION,
                   ...
                   help="system-prompt version for --strategy llm: 'v1' is rules "
                        "only, 'v2' adds the top hand-written bot's playbook "
    ...
    if args.strategy == "random":
        ...
        run_desc = (f"{smart.name} ({llm_client.model_name()}, prompt "
                    f"{smart.prompt_version}) vs Random")
        config_extra = {
